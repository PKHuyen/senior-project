        ...
        try:
            ...
            if 'google_oauth_credentials' in st.session_state:
                try:
                    ...
                    logging.error(f"Error with existing credentials: {e}")
            ...
            credentials_dict = {
                "installed": {  # Changed from "web" to "installed"
                    ...
                    "auth_provider_x509_cert_url": st.secrets["google_credentials"]["auth_provider_x509_cert_url"],
                    "client_secret": st.secrets["google_credentials"]["client_secret"],
                    "redirect_uris": ["http://localhost:8501/"]  # Use local redirect
                }
            }
            ...
            if st.button('Submit'):
                try:
                    ...
                    st.error(f'Authentication error: {str(e)}')
                    logging.error(f"Authentication error details: {traceback.format_exc()}")
                    return None
        ...
        try:
            ...
            try:
                ...
                file_metadata = drive_service.files().get(
                    fileId=bin_clip_file, 
                    fields='id, name, mimeType'
                ).execute()
                logging.info(f"Successfully accessed file: {file_metadata.get('name', 'unknown')}")
            except Exception as e:
                error_msg = f"Error accessing binary file {bin_clip_file}: {str(e)}"
            ...
            error_msg = f"Search engine initialization error: {str(e)}\n{traceback.format_exc()}"
            logging.error(error_msg)
            st.error(error_msg)
            return False
    return True
